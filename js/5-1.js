/*
--ATTEMPT 1--
* Got distracted on Dec 5th so this one took a while to finish. I don't remember exactly what I was thinking at the
start but I landed on a solution where each page has a record of what pages are allowed before it and what pages after.
That allows us to iterate over each page and validate that every subsequent page does not appear on the current page's
"before" list
*
 */
...
function doChallenge () {
  ...
  for (const [a, b] of data.rules) {
    ...
    formatted[a].bef.add(b)
    formatted[b].aft.add(a)
  }

  for (const pg in formatted) {
    formatted[pg].bef = Array.from(formatted[pg].bef)
